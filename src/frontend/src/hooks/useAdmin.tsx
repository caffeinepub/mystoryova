...
export function AdminProvider({ children }: { children: ReactNode }) {
  ...
  // Actor is only non-null after useActor has verified the canister is reachable
  const isActorReady = !!actor && !isFetching;

  const login = useCallback(
    async (password: string) => {
      if (!actor) throw new Error("Backend is still connecting. Please wait.");
      // No extra retry wrapper needed — by the time actor is non-null,
      // the canister is already confirmed reachable by useActor.
